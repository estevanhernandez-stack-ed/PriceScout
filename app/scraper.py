...
class Scraper:
    ...
    def _load_ticket_types(self):
        ...
        try:
            ...
            logger.error(f"Could not load ticket_types.json: {e}")
    ...
    def _parse_ticket_description(self, description: str, showing_details: dict | None = None) -> dict:
        ...
        normalized_desc = ' ' + description.lower().replace('−', '-').strip() + ' '
        
        found_amenities = []
        # --- Pass 1: Find and extract all known amenities ---
        for amenity, patterns in self.amenity_map_re.items():
            for pattern in patterns:
                if pattern.search(normalized_desc):
                    found_amenities.append(amenity)
                    # Replace the found keyword with a space to prevent re-matching.
                    normalized_desc = pattern.sub(' ', normalized_desc)
                    break # Move to the next amenity type once one of its keywords is found.

        # --- Pass 2: Find the best matching base ticket type ---
        found_base_type = None
        best_match_len = 0
        
        # Sort base types by the length of their longest keyword, descending.
        # This ensures "General Admission" is checked before "General" or "Admission".
        sorted_base_types = sorted(
            self.base_type_map_re.items(),
            key=lambda item: max(len(p.pattern) for p in item[1]),
            reverse=True
        )
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    if match_len > best_match_len:
                        ...
                        # Immediately replace to prevent it from being part of another match.
                        normalized_desc = pattern.sub(' ', normalized_desc, 1)

        # --- Pass 3: Handle remaining parts ---
        # Clean up the remaining description by removing ignored terms and extra characters.
        for term in self.ignored_amenities:
            normalized_desc = re.sub(r'\b' + re.escape(term) + r'\b', ' ', normalized_desc, flags=re.IGNORECASE)
        
        # Tokenize the rest of the string.
        remaining_parts = [part.strip() for part in normalized_desc.split() if part.strip()]

        # If no base type was found, the first significant remaining part becomes the base type.
        if not found_base_type and remaining_parts:
            # We'll take the first part as the base type.
            found_base_type = remaining_parts.pop(0).title()
            # Log this newly found type for review.
            try:
        ...
        # Any other remaining parts are treated as potential new amenities.
        if remaining_parts:
            for part in remaining_parts:
                # Add as a title-cased amenity if it's not already in the list.
                new_amenity = part.title()
                if new_amenity not in found_amenities:
                    found_amenities.append(new_amenity)

        # Final fallback if no base type could be determined.
        if not found_base_type:
            found_base_type = "Unknown"
            # Log the original description as it's completely unparsable.
            try:
                database.log_unmatched_ticket_type(description, "Unknown", context)
            except Exception as e:
                print(f"    [DB-WARN] Failed to log unparsable description '{description}'. Reason: {e}")

        return {
            "base_type": found_base_type,
            "amenities": sorted(list(set(found_amenities)))
        }
        ...
        async def _search(page_to_use: 'Page'): # type: ignore
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(f"Screenshot of the failure page saved to: {screenshot_path}")
                except Exception as screenshot_error:
                    # Sanitize exception details before logging
                    sanitized_error = security_config.sanitize_log_data(str(screenshot_error))
                    logger.error(f"Failed to take screenshot: {sanitized_error}")
            return results
        ...
        async def _search(page: 'Page'): # type: ignore
            try:
                ...
                try:
                    ...
                    # Sanitize exception details before logging
                    sanitized_error = security_config.sanitize_log_data(str(screenshot_error))
                    logger.error(f"Failed to take a debug screenshot. Reason: {sanitized_error}")
            return results
    ...
    async def _get_prices_and_capacity(self, page: 'Page', showing_details: dict) -> dict:
        ...
        try: # Reverting to the known-good logic from the working copy.
            ...
            # Attempt to get HTML content even if page.goto failed
            try:
                html_content = await page.content()
            except Exception as content_error:
                # Sanitize exception details before logging
                sanitized_error = security_config.sanitize_log_data(str(content_error))
                logger.warning(f"Failed to retrieve page content in exception handler: {sanitized_error}")
                html_content = "" # Ensure html_content is empty if retrieval fails
            if html_content:
                os.makedirs(DEBUG_DIR, exist_ok=True)
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                ...
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(html_content)
                logger.debug(f"Saved failing HTML for _get_prices_and_capacity to {filepath}")
    ...
    def _save_debug_html(self, html_content, prefix="debug"):
        ...
        logger.debug(f"Saved failing HTML to {filepath}")
    ...
    async def scrape_details(self, theaters: list, selected_showtimes: dict, status_container: list | None = None) -> tuple[list, list]:
            ...
            if theater_name in daily_selections:
                for film, times in daily_selections[theater_name].items():
                    for time_str, showing_info in times.items():
                        ...
                        # Market/CompSnipe: {showtime: [showing1, showing2, ...]} (list)
                        # Poster: {showtime_format: {url, format}} (dict)
                        if isinstance(showing_info, list):
                            # Market/CompSnipe mode - list of showing dicts
                            for showing in showing_info:
                                showings_to_scrape.append({**showing, "theater_name": theater_name, "play_date": date_str})
                        else:
                            # Poster mode - single showing dict
        ...
        async with async_playwright() as p:
            ...
            page_pool = [await context.new_page() for _ in range(len(showings_to_scrape))]
            ...
            for result in results_list:
                if result is not None and isinstance(result, list):
                    ...
                    # Sanitize exception details before logging
                    sanitized_error = security_config.sanitize_log_data(str(result))
                    logger.error(f"A price scraping worker failed with an exception: {sanitized_error}")
